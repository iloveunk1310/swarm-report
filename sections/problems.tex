...
The travelling saleman problem (TSP) là một bài toán kinh điển trong lý thuyết đồ thị và có rất nhiều ứng dụng trong thực tế và kĩ thuật.\cite{lawler1985tsp,laporte1992tsp}
...
Nó có thể được phát biểu dưới dạng đồ thị như sau: Cho một đồ thị vô hướng có trọng số, tìm chu trình ngắn nhất xuất phát từ một đỉnh bất kì, đi qua tất cả các đỉnh đúng một lần và quay về điểm xuất phát (chu trình hamilton ngắn nhất). Đây là một bài toán NP-hard\cite{garey1979computers}, có thể hiểu đơn giản là chúng ta chưa thể tìm ra lời giải đa thức chính xác cho bài toán này. Vì vậy, các phương pháp metaheuristic được đưa ra nhằm tìm ra lời giải chính xác gần đúng trong thời gian hợp lý.
...
Graph Coloring (GC) là bài toán tô màu đồ thị: cho một đồ thị vô hướng $G = (V,E)$, mục tiêu là gán màu (số nguyên) cho mỗi đỉnh sao cho hai đỉnh kề nhau không trùng màu. Bài toán tối ưu kinh điển là tìm \emph{chromatic number} $\chi(G)$ — số màu ít nhất cần dùng.\cite{jensen1995graphcoloring,garey1979computers}
...
\]
với nghiệm tối ưu toàn cục tại $x^\star = 0$, $f(x^\star) = 0$. Miền thường dùng là $[-5.12, 5.12]^d$ hoặc $[-100,100]^d$.\cite{dejong1975analysis}
...
\]
với nghiệm tối ưu toàn cục tại $x^\star = (1,1,\dots,1)$, $f(x^\star) = 0$. Miền thường dùng là $x_i \in [-5, 10]$.\cite{rosenbrock1960automatic}
...
\subsubsection{Rastrigin function}\cite{rastrigin1974systems}
...
+ a + e,
\]
với các giá trị khuyến nghị $a = 20$, $b = 0.2$, $c = 2\pi$, và miền điển hình $x_i \in [-32.768, 32.768]$. Nghiệm tối ưu toàn cục tại $x^\star = 0$, $f(x^\star) = 0$.\cite{ackley1987connectionist}
