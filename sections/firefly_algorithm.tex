...
Biểu diễn nghiệm bằng $\mathbf{b}_i^{(t)} \in \{0,1\}^n$. Các quy tắc di chuyển như sau.

\subsubsection*{(a) Di chuyển có hướng (Directed movement)}

Với mỗi đom đóm $j$ sáng hơn $i$:
\begin{enumerate}[leftmargin=1.6em]
    \item Xác định tập vị trí khác biệt:
    \[
        D = \{k \mid b_{i,k} \neq b_{j,k}\}.
    \]

    \item Chọn ngẫu nhiên \emph{tối đa} $m_{\max}$ vị trí từ $D$.

    \item Lật bit tại các vị trí được chọn sao cho:
    \[
        b_{i,k} \leftarrow b_{j,k}.
    \]
\end{enumerate}

\subsubsection*{(b) Nhiễu ngẫu nhiên (Random noise)}
...
\subsubsection*{(c) Sửa nghiệm vi phạm (Repair vs Penalty)}

Benchmark hỗ trợ hai chiến lược xử lý ràng buộc:
\begin{itemize}[leftmargin=1.6em]
    \item \textbf{Repair strategy}: Sau khi di chuyển, nếu nghiệm vi phạm ràng buộc sức chứa,
          loại dần các vật phẩm có tỷ số $v_k / w_k$ thấp nhất (greedy removal) cho đến khi khả thi.

    \item \textbf{Penalty strategy}: Nghiệm vi phạm nhận penalty lớn trong fitness,
          cho phép exploration trong không gian infeasible.
\end{itemize}
